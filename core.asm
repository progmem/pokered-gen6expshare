...
.playermonnotfaint
	...
	xor a
	ld [wBattleResult], a
; Gen VI-style EXP Share
; First, give all participating Pokemon experience.
.giveFullExperience
	xor a
	ld [wBoostExpByExpAll], a
	callfar GainExperience
; Next, build a mask of bits, setting one bit high for each party member we have.
.buildPartyMask
  ld a, [wPartyCount]
  ld b, $0
.partyMaskLoop
  ...
  cp $0
  jp nz, .partyMaskLoop
; Next, XOR our participating Pokemon flags using the mask we built.
; This gives us all non-participating Pokemon.
.checkForHalfExperience
  ld a, [wPartyGainExpFlags]
	xor b
  ld [wPartyGainExpFlags], a
; If this value is 0, no non-participating Pokemon exist and we can skip.
	cp $0
	jp z, .skipHalfExperience
; Otherwise, we toggle the MSB in the participating flags to change how GainExperience works.
; Consider this bit a "grant 50%" flag.
  xor $80
  ld [wPartyGainExpFlags], a
	jpfar GainExperience
.skipHalfExperience
	ret
; These nops serve as padding to keep everything else in the ROM aligned.
  nop
  nop
  nop
  nop
