...
; Divide enemy base stats, catch rate, and base exp by 2 if we're granting 50%.
DivideExpDataByNumMonsGainingExp:
; First, check if our "grant 50%" flag is set. If so, we divide by 2.
	ld a, [wPartyGainExpFlags]
  and $80
  cp $80
  jp z, .divideExpShare
  ret
; Next, we need to unset our flag to avoid edge cases
.divideExpShare
	ld a, [wPartyGainExpFlags] ; Grab party flags
	xor $80
	ld [wPartyGainExpFlags], a
; Once that's done, we load up our enemy stats and begin dividing like the original code.
; Well, mostly.
	ld hl, wEnemyMonBaseStats
...
.divideLoop
	...
	ld a, [hl]
	ldh [hDividend + 1], a
; We're dividing by a fixed value here, 2. Otherwise, everything else here is the same.
	ld a, $2
	ldh [hDivisor], a
	ld b, $2
	call Divide
